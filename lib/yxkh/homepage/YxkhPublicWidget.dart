...
// YxkhPublicWidget 一线考核文件
class YxkhPublicWidget extends StatelessWidget {
  final HomePageBlocImpl bloc;

  YxkhPublicWidget({this.bloc});
          ...
          child: IconButton(
            ...
            onPressed: () async {
              if (App.hasAuthority(context, "考核办")) {
                var data = await UserAPI.saveFileToDB("public");
                bloc.onAddPublicFile();
                // 更新显示的数据
            ...
            onPressed: () {
              App.showAlertDialog(
                  ...
                  Container(
                    ...
                    height: 100,
                    child: TextField(
                      controller: TextEditingController(text: bloc.publicFilename ?? ""),
                      onChanged: (val) {
                        bloc.publicFilename = val;
                      },
                      decoration: InputDecoration(contentPadding: EdgeInsets.all(10), hintText: "文件名搜索"),
                    ),
                  ), callback: () {
                bloc.onSearchPublicFile();
              });
            },
          ),
        ),
                                  ...
                                  switch (item.menuTitle) {
                                    ...
                                    case "删除":
                                      ...
                                      if (App.hasAuthority(context, "考核办") ||
                                        ...
                                        bloc.onDelPublicFile(snapshot.data[index]);
                                        return;
                                      }
