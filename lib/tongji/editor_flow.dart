          ...
          SliverGrid(
            delegate: SliverChildBuilderDelegate(
              (BuildContext context,int index) {
                return Center(
                  child: EditorFlowCard(route: '${Routes.Editor_Detail}?username=${dataList[index]["username"]}',
                    avatar: dataList[index]["avatar"],title: '${dataList[index]["realname"]}/ ${dataList[index]["username"]}',
...
class _ESearchBoxWidgetState extends State<ESearchBoxWidget> with TickerProviderStateMixin{
    ...
    void initState(){
      ...
      _tabs=<RouteHandler>[
        RouteHandler(title: '昨天', icon: Icons.directions_bike, handler:({Map<String,dynamic> params}){
          start = '${DateTime.now().subtract(Duration(days:1))}'.substring(0,10);
          end = start;
          callback({"start": start,"end": end});
        }),
        RouteHandler(title: '最近7天', icon: Icons.directions_boat, handler:({Map<String,dynamic> params}){
          start = '${DateTime.now().subtract(Duration(days:6))}'.substring(0,10);
          end = '${DateTime.now()}'.substring(0,10);
          callback({"start": start,"end": end});
        }),
        RouteHandler(title: '最近30天', icon: Icons.directions_bus, handler:({Map<String,dynamic> params}){
        ...
        RouteHandler(title: '导出', icon: Icons.directions_bus, handler:({Map<String,dynamic> params}){
          // callback({"method":"export/editor/flowAndManuscriptNumLastMonth"});
           showExportDialog();
        }),
        RouteHandler(title: '导出上月', icon: Icons.directions_bus, handler:({Map<String,dynamic> params}){
