...
class UserinfoCard extends StatefulWidget{
  ...
  UserinfoCard({Key key,this.userid,this.wxuserid}):super(key:key);
  @override
...
class _UserinfoCardState extends State<UserinfoCard>{
  ...
  void showDepartment(){
    App.showAlertDialog(context,Text('添加分管部门'),TreeWidget(data: departments,multiple: true,callback: (val){
  ...
  void alertAddLable(){
    ...
    App.showAlertDialog(context,Text("添加角色"),
      FilterChipGroup(labels: allLabels,
      callback: (val){
          if (val==null) return;
          var l=val.map((label){
  ...
  Widget build(BuildContext context) {
    if (_data==null || _data.length==0) return Center(child: Text('用户信息为空'),);
        ...
        slivers: <Widget>[
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Row(
                children: <Widget>[
                  Text('用户信息',style: textStyle,),
                  Padding(padding: EdgeInsets.only(left: 10),),
                  user["is_leader"]==1?Text("部门领导",style: TextStyle(color: Colors.blue),):Container(),
                ],
              ),
            ),
          ),
          SliverToBoxAdapter(
            child: ListTile(
              leading: CircleAvatar(
                radius: 30,
                child: user["avatar"]==null?Text(user["name"]):
                Image.network(
                  user["avatar"],
                ),
              ),
