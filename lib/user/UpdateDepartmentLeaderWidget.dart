  ...
  void setItems(){
    ...
    items.add(ListTile(
      leading: FlatButton.icon(
        ...
        label: Text("分管领导"),
        onPressed: (){
          App.showAlertDialog(context,Text("添加分管领导"),UserAutocompleteWidget(callback: (user){
    ...
    items.add(ListTile(
      leading: FlatButton.icon(
        ...
        label: Text("部门领导"),
        onPressed: (){
          App.showAlertDialog(context,Text("添加部门领导"),UserAutocompleteWidget(callback: (user){
            // print("id:${user.id},name:${user.name}");
            insert(user,5);
          },));
        },
    ...
    items.addAll(leaders.map((l)=>ListTile(
            ...
            title: Row(children: <Widget>[
              ...
              Padding(
                padding: EdgeInsets.only(right: 10),
                child: Text(""+_rolename(l.role),style: TextStyle(fontSize: 10,color: Colors.grey),),
              ),
              IconButton(
